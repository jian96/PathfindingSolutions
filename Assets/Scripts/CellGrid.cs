...
public class CellGrid
{
    ...
    public int Index(int x, int z) => z * gridSize + x;
    public Vector2Int Coordinates(int index) => new Vector2Int(index % gridSize, index / gridSize);

    public CellGrid(int _gridSize, int _cellSize, Vector3 _origin, int[] map)
    {
        gridSize = _gridSize;
        gridOrigin = _origin;
        cellSize = _cellSize;
        ...
        for (int z = 0; z < gridSize; z++)
        {
            for (int x = 0; x < gridSize; x++)
            {
                ...
                cells[index] = new Cell(index, cellSize, cellPos, map[index]);
            }
        }
    }
